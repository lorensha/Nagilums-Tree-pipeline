...
for combined_path in combined_files:
    ...
    df_percolator = pd.read_csv(percolator_path, sep="\t")

    # Concatenate
    df_result = pd.concat([df_combined, df_percolator], ignore_index=True)
    ...
    print(f"Combined: {sample_id} -> {output_path} ({df_result.shape[0]} rows)")