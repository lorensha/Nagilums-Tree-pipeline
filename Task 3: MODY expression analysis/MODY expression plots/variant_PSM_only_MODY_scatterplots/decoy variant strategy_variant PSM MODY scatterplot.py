...
stem_file_01_df = stem_file_01_df[stem_file_01_df['psm_type'] != 'canonical']
print(stem_file_01_df)
...
stem_file_02_df = stem_file_02_df[stem_file_02_df['psm_type'] != 'canonical']
# separate matching gene names if there are multiple entries and make each gene into its own row with duplicate info
# reset index for PSMIs and make new column names (avoid redundancy later)
...
stem_file_df_03 = stem_file_df_03[stem_file_df_03['psm_type'] != 'canonical']
# separate matching gene names if there are multiple entries and make each gene into its own row with duplicate info
# reset index for PSMIs and make new column names (avoid redundancy later)
...
stem_file_df_04 = stem_file_df_04[stem_file_df_04['psm_type'] != 'canonical']
# separate matching gene names if there are multiple entries and make each gene into its own row with duplicate info
# reset index for PSMIs and make new column names (avoid redundancy later)
...
stem_file_df_05 = stem_file_df_05[stem_file_df_05['psm_type'] != 'canonical']
# separate matching gene names if there are multiple entries and make each gene into its own row with duplicate info
# reset index for PSMIs and make new column names (avoid redundancy later)
...
stem_file_df_06 = stem_file_df_06[stem_file_df_06['psm_type'] != 'canonical']
# separate matching gene names if there are multiple entries and make each gene into its own row with duplicate info
# reset index for PSMIs and make new column names (avoid redundancy later)
...
stem_file_df_07 = stem_file_df_07[stem_file_df_07['psm_type'] != 'canonical']
# separate matching gene names if there are multiple entries and make each gene into its own row with duplicate info
# reset index for PSMIs and make new column names (avoid redundancy later)
...
stem_file_df_08 = stem_file_df_08[stem_file_df_08['psm_type'] != 'canonical']
# separate matching gene names if there are multiple entries and make each gene into its own row with duplicate info
# reset index for PSMIs and make new column names (avoid redundancy later)
...
stem_file_df_09 = stem_file_df_09[stem_file_df_09['psm_type'] != 'canonical']
# separate matching gene names if there are multiple entries and make each gene into its own row with duplicate info
# reset index for PSMIs and make new column names (avoid redundancy later)
...
stem_file_df_10 = stem_file_df_10[stem_file_df_10['psm_type'] != 'canonical']
# separate matching gene names if there are multiple entries and make each gene into its own row with duplicate info
# reset index for PSMIs and make new column names (avoid redundancy later)
...
# stem_file_df_11 = stem_file_df_11[stem_file_df_11['psm_type'] != 'canonical']
...
plt.title('Decoy variant strategy: PEP distribution of variant PSMs from classified iPSCs of possible MODY genes')
plt.legend(MODY_df.columns, bbox_to_anchor=(1, 1), loc='upper left', title='iPSCs growth stage and type')
plt.savefig('variant iPSCs 14 MODY pep distribution decoy variant search.png', bbox_inches='tight', dpi=600)
plt.show()
