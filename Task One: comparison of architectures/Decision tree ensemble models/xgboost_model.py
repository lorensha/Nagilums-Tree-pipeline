...
# *label encoding setup for XGBoost only as scaling measure* *used for all label arrays for training* *3 instances*
...
def xgboost_main_predictor(psm_input_file):
    ...
    print('check: ', target_psms_within_fdr_threshold)
    ...
    for fold_01, (train_index_01, test_index_01) in (
        ...
        for fold_02, (train_index_02, test_index_02) in (
            ...
            nested_cross_validation_training_labels = (
                label_encoding.fit_transform(nested_cross_validation_training_labels))

            grid = {'gamma': [1, 5], 'min_child_weight': [1, 5], 'max_depth': [3, 6], 'subsample': [0.5, 1.0],
                    'colsample_bytree': [0.5, 1.0]}

            xgboost_hyperparameter_search = GridSearchCV(estimator=XGBClassifier(), param_grid=grid)

            xgboost_hyperparameter_search.fit(nested_cross_validation_training_feature,
                                              nested_cross_validation_training_labels)
            best_parameters_compiled = xgboost_hyperparameter_search.best_params_
        ...
        training_single_feature_labels_sample = label_encoding.fit_transform(training_single_feature_labels_sample)
        ...
        xgboost_cross_validation_model = XGBClassifier(**best_parameters)
        xgboost_cross_validation_model.fit(training_single_feature_variable_sample,
                                           training_single_feature_labels_sample)

        cross_validation_predictions = (xgboost_cross_validation_model.predict
                                        (testing_single_feature_variable_sample))
        cross_validation_probability_scores = (xgboost_cross_validation_model.predict_proba
                                               (testing_single_feature_variable_sample))
    ...
    training_all_feature_labels_sample = label_encoding.fit_transform(training_all_feature_labels_sample)
    ...
    # fit model and obtain psm class type predictions and probability scores
    xgboost_final_model = XGBClassifier(**best_hyperparameters)
    xgboost_final_model.fit(training_all_feature_variables_sample, training_all_feature_labels_sample)
    main_predictor_predictions = xgboost_final_model.predict(testing_all_feature_variables_sample)
    main_predictor_probability_scores = (xgboost_final_model.predict_proba
...
def counting_system(input_psms_original, targets_count_list, predictor_dataframe_list):
    ...
    predictor_output = xgboost_main_predictor(input_psms_original)
    ...
    if last_iteration_target_count >= second_last_iteration_target_count:
        ...
        best_predictor_output_confusion_matrix.to_csv('confusion matrix xgboost.csv')
