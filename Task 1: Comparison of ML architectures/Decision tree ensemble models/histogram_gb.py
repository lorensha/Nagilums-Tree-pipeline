...
def histogram_gb_main_predictor(psm_input_file):
    ...
    for fold_01, (train_index_01, test_index_01) in (
        ...
        for fold_02, (train_index_02, test_index_02) in (
            ...
            grid = {'learning_rate': [0.1, 1.0], 'max_leaf_nodes': [31, 50], 'min_samples_leaf': [100, 150],
                    'max_iter': [200, 50]}

            histogram_gb_hyperparameter_search = \
                (GridSearchCV(estimator=HistGradientBoostingClassifier(warm_start=False), param_grid=grid))

            histogram_gb_hyperparameter_search.fit(nested_cross_validation_training_feature,
                                                   nested_cross_validation_training_labels)
            best_parameters_compiled = histogram_gb_hyperparameter_search.best_params_
        ...
        histogram_gb_cross_validation_model = HistGradientBoostingClassifier(warm_start=False, **best_parameters)
        histogram_gb_cross_validation_model.fit(training_single_feature_variable_sample,
                                                training_single_feature_labels_sample)

        cross_validation_predictions = (histogram_gb_cross_validation_model.predict
                                        (testing_single_feature_variable_sample))
        cross_validation_probability_scores = (histogram_gb_cross_validation_model.predict_proba
                                               (testing_single_feature_variable_sample))
    ...
    # fit model and obtain psm class type predictions and probability scores
    histogram_gb_final_model = HistGradientBoostingClassifier(warm_start=False, **best_hyperparameters)
    histogram_gb_final_model.fit(training_all_feature_variables_sample, training_all_feature_labels_sample)
    main_predictor_predictions = histogram_gb_final_model.predict(testing_all_feature_variables_sample)
    main_predictor_probability_scores = (histogram_gb_final_model.predict_proba
...
def counting_system(input_psms_original, targets_count_list, predictor_dataframe_list):
    ...
    predictor_output = histogram_gb_main_predictor(input_psms_original)
    ...
    if last_iteration_target_count >= second_last_iteration_target_count:
        ...
        best_predictor_output_confusion_matrix.to_csv('confusion matrix histogram gb.csv')
